...
<?php
    ...
    if(isset($_POST['login_btn'])){
        ...
        $check_database_query = mysqli_query($conn, "SELECT * FROM admin WHERE adminname='$Username' AND password='$password'")or die(mysqli_error($conn));
        $check_login_query = mysqli_num_rows($check_database_query);
    
        if($check_login_query == 1){
            $row = mysqli_fetch_array($check_database_query) or die(mysqli_error($conn));
            $username = $row['adminname'];
    
            $user_closed_query = mysqli_query($conn,"select * from admin where adminname='$Username'");    
