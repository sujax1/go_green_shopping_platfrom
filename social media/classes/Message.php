...
<?php 
    ...
    class Message {
        ...
        public function getLastMsg($userLoggedIn, $otheruser){
            ...
            $query = mysqli_query($this->conn, "select body, user_to, date from messages where (user_to='$userLoggedIn' and user_from='$otheruser') or (user_from='$userLoggedIn' and user_to='$otheruser') ORDER BY id DESC LIMIT 1");
        ...
        public function sendMessage($user_to, $body, $date){
            if ($body != "") {            
                $userLoggedIn = $this->user_obj->getUsername();
                $query = mysqli_query($this->conn, "insert into messages values('','$user_to','$userLoggedIn','$body','$date','no','no','no')")or die(mysqli_error($this->conn));
            }
        ...
        public function getMessages($otheruser){
            ...
            $get_msg_query = mysqli_query($this->conn, "select * from messages where (user_to='$userLoggedIn' and user_from='$otheruser') or (user_from='$userLoggedIn' and user_to='$otheruser')");
