...
<?php
    ...
    {
        $user = $_POST['search'];
        $query = mysqli_query($conn, "delete from users where username='$user'") or die("No User Found");
        $post_query = mysqli_query($conn, "delete from post where added_by='$user'")or die("can not Delete posts");
        if($query){
