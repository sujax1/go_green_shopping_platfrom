...
<body>
    ...
    <?php
        ...
        $get_like = mysqli_query($conn, "select likes, added_by from posts where id='$post_id'");
        ...
        if(isset($_POST['like_btn'])){
            $total_likes++;
            $query = mysqli_query($conn, "update posts set likes='$total_likes' where id='$post_id'")or die(mysqli_error($conn));
            $total_user_likes++;
            $user_likes = mysqli_query($conn, "update users set num_likes='$total_user_likes' where username='$user_liked'");
            $insert_query = mysqli_query($conn, "insert into likes values('','$userLoggedIn','$post_id')");
        }
        ...
        if(isset($_POST['unlike_btn'])){
            ...
            $total_user_likes--;
            $user_likes = mysqli_query($conn, "update users set num_likes='$total_user_likes' where username='$user_liked'");
            $insert_query = mysqli_query($conn, "delete from likes where username='$userLoggedIn' and post_id='$post_id'");
        }

        //chech previus likes
        $check_query = mysqli_query($conn, "select * from likes where username='$userLoggedIn' AND post_id='$post_id'")or die(": ( ".mysqli_error($conn));
