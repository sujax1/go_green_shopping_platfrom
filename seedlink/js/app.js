...
(function() {
    ...
    function showTotals() {
        ...
        document.querySelector('.item-tlota').textContent = finalMoney;
...
function deleteItemFromCart(event) {
    if (event.target.classList.contains('fa-trash')) {
        ...
        const price = parseFloat(removeItem.querySelector('.cart-item-price').textContent);
        removeItem.remove();
        updateTotal(-price); // Reduce the total by the price of the removed item
    }
}
...
function updateTotal(itemPrice) {
    ...
    const currentItemCount = parseInt(itemCountElement.textContent);
...
const cart = document.getElementById('cart');
cart.addEventListener('click', deleteItemFromCart);
